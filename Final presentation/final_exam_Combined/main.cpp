...
private:
	int actative_num;
public:
	Pretender(People* newplayer):Monster(newplayer){
		player = newplayer;
		actative_num = 0;
	}
	void Print(){
...
private:
	...
	clock_t finish_v;
public:
	Vampire(People *newplayer):Monster(newplayer){
		player = newplayer;
		bloodtime = 0;
	}
	void Print(){
	...
	void Blood(){
		double duration;
		finish_v = clock();
		duration=(double)(finish_v - start_v) / CLOCKS_PER_SEC;
			if(duration>=1&&bloodtime>0){
				player->GetDamage(1);
				bloodtime--;
				start_v = clock();
		}
...
private:
	...
	int JudgeTime;
public:
	WallMonster_Wall *next;
	WallMonster_Wall(int i,int j){
		x = i;
		y = j;
		start=clock();
		Map::SetItem(x,y,WALL);
		gotoxy(x,y);
		cout<<"*";
		JudgeTime=0;
	}
...
public:
	...
	void Insert(WallMonster_Wall *newWallMonster_Wall){
		if(p==NULL){
			p = newWallMonster_Wall;
			p->next = p;
		}else{
			newWallMonster_Wall->next = p->next;
			p->next = newWallMonster_Wall;
		}
	}
	void ActionList(){
		WallMonster_Wall *n = p;
		...
		do{
			n->Action();
			if(n->next->GetJudgeTime()==1){
				DeleteList(n,n->next);
			}
			n = n->next;
		}while(n != p);
	}
	void DeleteList(WallMonster_Wall *a,WallMonster_Wall *b){
		p=a;
		WallMonster_Wall *d = b;
		a ->next = a->next->next;
		delete d;
	}
...
class WallMonster:public Monster{
private:
	Linkedlist_Wall *listwall = new Linkedlist_Wall;
public:
	WallMonster(People *newplayer):Monster(player){
		player=newplayer;
		do{
			x=rand()%(WIDTH-1)+1;
			y=rand()%(HEIGHT-1)+1;
		}while(Map::GetMap(x,y)==1);
	}
	~WallMonster(){
		gotoxy(x,y);
		cout<<" ";
	...
	void Action(){
		...
		if(OriginX!=x||OriginY!=y){
			W = new WallMonster_Wall(OriginX,OriginY);
			listwall->Insert(W);
		}
		listwall->ActionList();
...
public:
	...
	void Action(Linkedlist_Node *listnode){
		...
		if(duration>=MONSTER_GENRATEION_TIME){
			//// let the monsters generate randomly
            int i = rand()%5;
            switch(i){
	            case 0:
	            ...
	            case 3:
	            	newMonster = new Vampire(player);
	            	listnode->Insert(newMonster);
	            	start = clock();
	            	break;

	            case 4:
	            	newMonster = new WallMonster(player);
	            	listnode->Insert(newMonster);
	            	start = clock();
	            	break;
	           }
